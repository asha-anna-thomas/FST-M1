...
public class ConsumerTest {
    ...
    public V4Pact createPostFragment(PactDslWithProvider builder) {
        ...
        DslPart reqResBody = new PactDslJsonBody()
                .numberType("id", 123)
                .stringType("firstName", "Margeret")
                .stringType("lastName", "Klein")
                .stringType("email", "MaggyKlein@example.com");
    ...
    public V4Pact createGetFragment(PactDslWithProvider builder) {
        ...
        DslPart resBody = new PactDslJsonBody()
                ...
                .stringType("firstName", "Margeret")
                .stringType("lastName", "Klein")
                .stringType("email", "MaggtKlein@example.com");
        // Create the contract(Pact)
    ...
    public void postRequestTest(MockServer mockServer) {
        ...
        reqBody.put("id", 123);
        reqBody.put("firstName", "Margeret");
        reqBody.put("lastName", "Klein");
        reqBody.put("email", "MaggyKlein@example.com");

        // Send request, get response, assert response
        given().baseUri(mockServer.getUrl() + "/api/users").headers(headers).body(reqBody).log().all().
                when().post().
                then().statusCode(201).body("email", equalTo("MaggyKlein@example.com")).log().all();
    }
